...
class Processor():
    ...
    def train(self, epoch):
        ...
        input_tensor = torch.randn(2, 16, 512, 8).to(device)
        flops, params = profile(model, inputs=(input_tensor,))
        print(f"参数量(Paras): {params / 1e6:.2f} M")  # 转换为百万（M）
        print(f"FLOPs: {flops / 1e9:.2f} G")  # 转换为十亿（G）
        start = time.time()
        model(input_tensor)
        #if device.type == "cuda":
        #    torch.cuda.synchronize()
        end = time.time()
        one_time = (end - start) * 1000
        print(f"平均推理时间: {one_time:.2f} ms")
    ...
    def print_inf_log(self, epoch, mode):
        static = self.stat.show_accuracy('{}/{}_confusion_mat'.format(self.arg.work_dir, mode), epoch)
