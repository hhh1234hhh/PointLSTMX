...
class Motion(nn.Module):
    ...
    def select_ind(self, group_array, inputs, batchsize, in_dim, timestep, pts_num):
        ind = self.weight_select(group_array, pts_num)                                     #生成索引
        ret_group_array = group_array.gather(-2, ind.unsqueeze(1).unsqueeze(-1).
                                             expand(-1, group_array.shape[1], -1, -1,
                                                    group_array.shape[-1]))
        ret_group_array = ret_group_array.view(batchsize, in_dim, timestep * pts_num, -1)  #下采样，[b,c,t*n*下采样,knn]
        inputs = inputs.gather(-1, ind.unsqueeze(1).expand(-1, inputs.shape[1], -1, -1))   #原始inputs的下采样结果
        return ret_group_array, inputs, ind                                                #之后通过mlp和池化转换成需要的四维张量
    
    
    @staticmethod         #选出每批次，每帧距原点距离最大的的点的索引，只选一个，所以最后的输出结果是[b,knn]
    def weight_select(position, topk):
        # select points with larger ranges
        weights = torch.max(torch.sum(position[:, :3] ** 2, dim=1), dim=-1)[0]  #选取每帧中最大的一个点
