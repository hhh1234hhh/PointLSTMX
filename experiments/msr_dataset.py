...
class Dataset(data.Dataset):
    def __init__(self, framerate, phase="train", datatype="depth", inputs_type="pts"):
        ...
        self.prefix = "../dataset/msraction/msraction_npy/a{}_s{}_e{}_sdepth_pts.npy"
        self.r = re.compile('[ \t\n\r:]+')
    ...
    def __getitem__(self, index):
        ...
        label28 = int(splitLine[0]) - 1
        # label14 = int(splitLine[-4]) - 1
        input_data = np.load(
            self.prefix.format(splitLine[0], splitLine[1], splitLine[2]))[:, :, :7]
            #insert(self.prefix.format(splitLine[0], splitLine[1], splitLine[2], splitLine[3])))[:, :, :7]
        input_data = input_data[self.key_frame_sampling(len(input_data), self.framerate)]
        mean_3d = np.sum(input_data, axis=2)/3
        input_data = np.concatenate([input_data, mean_3d[:, :, np.newaxis]], axis=2)
    ...
    def get_inputs_list(self):
        prefix = "../dataset/msraction"
        if self.phase == "train":
            inputs_path = prefix + "/train.txt"
        if self.phase == "test":
            inputs_path = prefix + "/test.txt"
...
if __name__ == "__main__":
    dataloader = Dataset(framerate=80)
